...
const elements = {
  ...
  btnLoginMicrosoft: document.getElementById("btnLoginMicrosoft"),
  btnLogoutMicrosoft: document.getElementById("btnLogoutMicrosoft"),
...
function renderSites() {
  elements.siteSelect.innerHTML = '<option value="">Choisir...</option>';
...
async function handleEnrollUsers() {
  ...
  try {
    ...
    if (successCount > 0) {
      showNotification(
        `${successCount} inscription(s) enregistrée(s)`,
        "success"
      );
    }
    if (errorCount > 0) {
...
// Mettre à jour la liste des inscriptions en attente
function updatePendingList(user, site) {
  const pendingHTML = `
        <div class="pending-item">
            <div class="pending-item-name">${user.display_name}</div>
            <div class="pending-item-site">${site.name}</div>
            <div class="pending-item-date">${new Date().toLocaleDateString(
              "fr-FR"
            )} ${new Date().toLocaleTimeString("fr-FR")}</div>
        </div>
    `;

  if (elements.pendingList.innerHTML.includes("Aucune inscription")) {
    elements.pendingList.innerHTML = pendingHTML;
  } else {
    elements.pendingList.insertAdjacentHTML("afterbegin", pendingHTML);
  }
}
...
async function handleLoginMicrosoft() {
  ...
  if (result.success) {
    appState.microsoftConnected = true;
    elements.microsoftStatus.textContent = `✓ ${result.account.username}`;
    elements.btnLoginMicrosoft.classList.add("hidden");
    elements.btnLogoutMicrosoft.classList.remove("hidden");
...
async function handleLogoutMicrosoft() {
  await window.MicrosoftGraph.logoutMicrosoft();
  appState.microsoftConnected = false;
  elements.microsoftStatus.textContent = "";
  elements.btnLoginMicrosoft.classList.remove("hidden");
  elements.btnLogoutMicrosoft.classList.add("hidden");
...
elements.btnLoginMicrosoft.addEventListener("click", handleLoginMicrosoft);
elements.btnLogoutMicrosoft.addEventListener("click", handleLogoutMicrosoft);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Auto-connexion Supabase si pré-rempli
...
// Style pour l'animation spinning
